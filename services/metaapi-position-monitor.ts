...
export class MetaApiPositionMonitor {
  ...
  private async monitorAccount(account: AdminAccount) {
    ...
    if (!account.metaapi_account_id) {
      console.error(
        `❌ Compte ${account.account_number} n'a pas de MetaApi ID`,
      );
      return;
    }
    ...
    const interval = setInterval(async () => {
      try {
        ...
        console.error(
          `Erreur monitoring compte ${account.account_number}:`,
          error,
        );
      }
    }, 5000); // 5 secondes
  ...
  private async handleNewPosition(account: AdminAccount, position: Position) {
    ...
    console.log(
      `🆕 Nouvelle position détectée: ${position.symbol} ${position.type}`,
    );
  ...
  private async copyPositionToUserAccount(
    ...
    if (!sub || (sub.status !== "active" && sub.status !== "trialing")) {
      console.log(
        `⛔ User ${user.id} abonnement inactif (${sub?.status}), copie ignorée`,
      );
      return;
    }
    ...
    if (!userAccount.is_active) {
      console.log(
        `⛔ Compte ${userAccount.account_number} inactif, copie ignorée`,
      );
      return;
    }
    ...
    if (!userSymbol) {
      console.log(
        `⚠️ Pas de mapping pour ${standardSymbol} sur ${userAccount.broker_name}`,
      );
      return;
    }
    ...
    if (orderResult.success) {
      ...
      await supabase.from("copy_trades").insert({
        ...
        follower_ticket: orderResult.positionId
          ? parseInt(orderResult.positionId)
          : null,
        status: "opened",
      ...
      console.error(
        `❌ Échec ordre pour user ${userAccount.account_number}:`,
        orderResult.error,
      );
   ...
   */
  private async checkClosedPositions(
   ...
   */
  private async closeUserPositions(
    adminAccount: AdminAccount,
   ...
   */
  private async getStandardSymbol(
    brokerName: string,
    brokerSymbol: string,
  ): Promise<string | null> {
    ...
    if (symbol.includes("DJ30")) return "US30";
    if (symbol.includes("NAS100")) return "NAS100";
    if (symbol.includes("GER40") || symbol.includes("DE40")) return "GER40";
    const noSuffix = symbol.replace(/\+/g, "");
    if (noSuffix === "EURUSD" || noSuffix === "GBPUSD" || noSuffix === "USDJPY")
      return noSuffix;
