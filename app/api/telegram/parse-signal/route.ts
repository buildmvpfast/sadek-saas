...
async function executeTradesForSignal(signalId: string) {
  ...
  const { data: signal } = await supabase
    .from("telegram_signals")
    .select(
      "id, channel_id, signal_type, symbol, entry_price, stop_loss, take_profit, volume"
    )
    .eq("id", signalId)
    .single();
    ...
    if (mt5Account.broker_name) {
      ...
      if (supportedBrokers.includes(mt5Account.broker_name)) {
        ...
        if (symbolMapping) {
          ...
          console.log(
            `✅ Mapping: ${normalizedSymbol} → ${brokerSymbol} pour ${mt5Account.broker_name}`
          );
        } else {
          console.log(
            `⚠️ Pas de mapping trouvé pour ${normalizedSymbol} sur ${mt5Account.broker_name}, utilisation du symbole original`
          );
        }
      } else {
        console.log(
          `⚠️ Broker ${mt5Account.broker_name} non supporté, utilisation du symbole original`
        );
      }
    }
